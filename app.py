...
st.markdown("---")
st.subheader("📊 Аналитика фьючерса и рекомендуемый стоп 10% ATR (BYDFi, 5 псевдо-дней из 6×4H)")
...
if show_analysis:
    try:
        ...
        if matched_symbol is None:
            ...
            # --- забираем 4H свечи и агрегируем в псевдо-дневки по 6 свечей ---

            candles_per_day = 6
            days_for_atr = 5          # считаем ATR по 5 последним "дням"
            needed_4h = candles_per_day * days_for_atr  # 30 4H-свечей

            try:
                ohlcv_4h = exchange.fetch_ohlcv(matched_symbol, timeframe="4h", limit=needed_4h)
            except Exception as e_4h:
                st.error(
                    f"Не удалось получить 4H свечи (OHLCV, 4h) по {matched_symbol} на BYDFi:\n\n"
                    f"{e_4h}"
                )
                st.stop()

            if not ohlcv_4h or len(ohlcv_4h) < needed_4h:
                st.error(
                    f"Недостаточно 4H свечей для расчёта псевдо-дневного ATR({days_for_atr}). "
                    f"Нужно {needed_4h}, а получили {len(ohlcv_4h) if ohlcv_4h else 0}."
                )
                st.stop()
            ...
            # агрегируем каждые 6 свечей в одну "дневную"
            daily_rows = []
            for i in range(0, needed_4h, candles_per_day):
                chunk = df_4h.iloc[i:i + candles_per_day]
                if len(chunk) < candles_per_day:
                    continue

                day_time = int(chunk["time"].iloc[0])          # время первой свечи
                day_open = float(chunk["open"].iloc[0])
                day_close = float(chunk["close"].iloc[-1])
                day_high = float(chunk["high"].max())
                day_low = float(chunk["low"].min())
                day_volume = float(chunk["volume"].sum())

                daily_rows.append([day_time, day_open, day_high, day_low, day_close, day_volume])

            if len(daily_rows) < days_for_atr:
                st.error(
                    f"После агрегации получили только {len(daily_rows)} псевдо-дневок, "
                    f"этого недостаточно для ATR({days_for_atr})."
                )
            else:
                ...
                # --- стандартный ATR(days_for_atr) по псевдо-дневкам ---
                ...
                df_ohlc["tr3"] = (df_ohlc["low"] - df_ohlc["prev_close"]).abs()
                df_ohlc["tr"] = df_ohlc[["tr1", "tr2", "tr3"]].max(axis=1)
                atr = df_ohlc["tr"].rolling(window=days_for_atr).mean().iloc[-1]

                if pd.isna(atr) or atr <= 0:
                    st.error(f"Не удалось корректно посчитать ATR({days_for_atr}) по этому фьючерсу (4H→дневки).")
                else:
                    atr_10 = atr * 0.10           # 10% ATR
                    max_luft = atr_10 * 0.10      # 10% от рекомендуемого стопа = 1% ATR

                    # для оценки волатильности тоже используем агрегированные "дневки"
                    range_pct = (df_ohlc["high"] - df_ohlc["low"]) / df_ohlc["close"] * 100
                    avg_range = range_pct.mean()
                    ...
                    st.write(f"Найденный фьючерсный символ на BYDFi: **{matched_symbol}**")
                    st.write(f"Текущая цена: **{last_price:.4f} USDT**")
                    st.write(f"ATR({days_for_atr}) по псевдо-дневкам (6×4H): **{atr:.4f} USDT**")
...
with col_r2:
    risk_percent = st.number_input(
        ...
        max_value=10.0,
        step=0.01,
        help="Введи риск в процентах от депозита, который готов потерять в одной сделке."
    )
...
with col_p2:
    ...
    stop_price = st.number_input(
        ...
        value=float(suggested_sl),
        min_value=0.0001,
        help="Если выше считали ATR, сюда подставлен стоп по 10% ATR, можно скорректировать."
    )
...
>>>>>>> a5a90c7 (Update ATR logic and remove risk buttons)
